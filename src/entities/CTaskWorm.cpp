...
BitmapImage* __stdcall hookSetNameTextbox(CTaskWorm * worm, BitmapTextbox * box, char *text, int text_color, int color1, int color2, int * width, int * height, int opacity) {
	if(CTaskTeam * team = (CTaskTeam *) worm->parent) {
		if(DWORD ddmain = *(DWORD*)(worm->gameglobal2c + 0x24)) {
			...
			if(mymachine == team->owner_byte40 || (ddgame && *(BYTE*)(ddgame + 0x490))) {
...
void __declspec(naked) hookDrawWormTextboxes_patch1() {
	_asm {
